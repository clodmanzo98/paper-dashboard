...
c1.metric("SALDO ATTUALE", f"${saldo_attuale:.2f}")
...
c5.metric("PROFIT FACTOR", f"{(data['stats']['total_profit']/abs(data['stats']['total_loss'])):.2f}x")
...
with t1:
    ...
    fig_eq.update_layout(template="plotly_dark", height=500, margin=dict(l=0,r=0,b=0,t=30))
...
with t2:
    ...
    with col_r1:
        ...
        st.plotly_chart(px.histogram(df, x="pnl", color_discrete_sequence=['#00FF41'], template="plotly_dark"), use_container_width=True)
    with col_r2:
        st.subheader("Efficienza Segnale (Exit Reasons)")
        st.plotly_chart(px.pie(df, names='exit_reason', hole=0.5, template="plotly_dark"), use_container_width=True)
...
with t3:
    ...
    fig_lead = px.scatter(leaders, x="Win_Rate", y="Profitto_Tot", size="Trade_Tot", color="ROI_Medio", 
                          hover_name="leader", template="plotly_dark", size_max=60, labels={"Profitto_Tot":"Profitto Totale ($)", "Win_Rate":"Win Rate (%)"})
...
with t4:
    ...
    with c_t1:
        ...
        st.plotly_chart(px.bar(day_stats, x='giorno_sett', y='pnl', template="plotly_dark"), use_container_width=True)
    with c_t2:
        st.write("**Heatmap Oraria (Profitto Totale)**")
        hourly_stats = df.groupby('ora')['pnl'].sum().reset_index()
        st.plotly_chart(px.bar(hourly_stats, x='ora', y='pnl', template="plotly_dark"), use_container_width=True)
